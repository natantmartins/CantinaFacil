...
class ProdutosScreen extends React.Component{
  ...
  calcularFichas(preBruto){
    //Fazendo as fichas
    valorAtual=preBruto;//COMENTADO PRA TESTAR DEPOIS QUE TESTAR VOLTA
    //valorAtual=0
    textFichas='';
    fichasToBuy=[];
    j=0;
  ...
  calculos(){
    ...
    alert(valorFichas)
    count=0
    if(parseFloat(valorFichas)>0){
        
    
    while(valorFichas>0){
      count++
      valortmp=valortmp-0.15
      this.calcularFichas(roundN(valortmp,2))
      alert("Valor: "+roundN(valortmp,2)+"Sobra fichas: "+valorFichas+"contagem: "+count)
      //alert(valortmp)
      if(parseFloat(valorFichas)>0){
        ...
        for(i=0;i<fichasToBuy.length;i++){
          if(fichasToBuy[i].id==6){
            fichasToBuy[i].qntd=fichasToBuy[i].qntd+count
            this.setState({
